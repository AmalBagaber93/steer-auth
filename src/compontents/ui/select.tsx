...
function Select({
  ...
  return (
    <SelectIdContext.Provider value={id}>
...
function SelectTrigger({
  ...
  const contextId = React.useContext(SelectIdContext);
  return (
    <SelectPrimitive.Trigger
      suppressHydrationWarning
      id={id ?? contextId}
      className={cn(
        'border-input bg-background group-hover/select-container:border-ring data-placeholder:text-muted-foreground focus:ring-ring/20 hover:border-ring/50 focus:border-ring flex h-10 w-full items-center justify-between border px-3 py-2 text-sm shadow-sm transition-colors duration-150 focus:ring-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
        className
      )}
      {...props}
    >
